...
# Sections that carry no study value (FR + DE + IT housekeeping headings).
_SKIP_SECTIONS = {
    # FR
    "notes et références", "références", "voir aussi", "liens externes",
    "bibliographie", "articles connexes", "annexes", "sources", "notes",
    # DE
    "einzelnachweise", "weblinks", "siehe auch", "literatur", "anmerkungen",
    "quellen", "fußnoten",
    # IT
    "note", "voci correlate", "altri progetti", "bibliografia",
    "collegamenti esterni", "galleria d'immagini",
}
# Human ref prefix per theme (kept stable: FR matelotage units must not shift id).
_REF_PREFIX = {"matelotage": "Matelotage", "meteorologie": "Météo"}
# Leftover "[edit | edit source]" markers across languages, if any survive.
_EDIT_MARK = re.compile(r"\[\s*(modifier|bearbeiten|modifica)[^\]]*\]", re.I)
...
def parse(src: Source, manifest: dict) -> list[KnowledgeUnit]:
    ...
    lang = manifest.get("lang", src.lang)
    theme = src.pin_theme or src.default_theme or "matelotage"
    ref_prefix = _REF_PREFIX.get(theme, theme.capitalize())
    ...
    for page_title, info in manifest["pages"].items():
        ...
        def flush(section_name: str, paras: list[str], with_image: bool):
            ...
            if len(text) < 80 or section_name.lower() in _SKIP_SECTIONS:
                return
            ref = f"{ref_prefix} — {page_title} : {section_name}"
            assets = []
        ...
        for el in root.find_all(["h2", "h3", "p"], recursive=True):
            if el.name in ("h2", "h3"):
                ...
                buf = []
                section = el.get_text(" ", strip=True).replace("[modifier | modifier le code]", "")
                section = _EDIT_MARK.sub("", section).strip()
            elif el.name == "p":
